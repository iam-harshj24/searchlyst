...
export default function AgentPage({ user }) {
    ...
    const handleSend = async () => {
        ...
        const brandContext = user?.brandName ? ` for **${user.brandName}**` : '';
        const industryContext = user?.industry ? ` in the **${user.industry}** industry` : '';
        
        setMessages(prev => [...prev, { 
            role: 'assistant', 
            content: `Thanks for your question about "${userMessage.slice(0, 50)}..."\n\nI'm your AI assistant${brandContext}${industryContext}. I'm currently in **demo mode**, but here's what I know about your brand:\n\n${user?.domain ? `- **Website:** ${user.domain}` : ''}\n${user?.industry ? `- **Industry:** ${user.industry}` : ''}\n${user?.location ? `- **Location:** ${user.location}` : ''}\n${user?.competitors?.length ? `- **Competitors tracked:** ${user.competitors.length}` : ''}\n\n**What I can help with:**\n- Content optimized for ${user?.industry || 'your industry'}\n- AI search visibility tips for ${user?.brandName || 'your brand'}\n- Website audit recommendations\n- Competitor analysis insights\n\nConnect an AI API key in settings to enable live AI responses.`
        }]);
        setIsLoading(false);
    };
    ...
    const quickActions = [
        { icon: Sparkles, label: "Optimize for AI search", prompt: `Help me optimize ${brandName}'s content for better AI search visibility in ${industry}` },
        { icon: FileText, label: "Draft content", prompt: `Help me draft a LinkedIn post about ${industry} trends for ${brandName}` },
        { icon: Search, label: "Find trending topics", prompt: `What are the top trending topics in ${industry} right now?` },
        { icon: Search, label: "Audit my website", prompt: `Run a quick analysis on ${user?.domain || 'my website'} and suggest improvements` },
    ];
                        ...
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                                    msg.role === 'user' ? 'bg-red-600' : 'bg-[var(--surface-hover)] border border-[var(--border)]'
                                }`}>
                    ...
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="flex items-center gap-6 mt-3">
                            <button 
                                ...
                                className="flex items-center gap-2 text-sm"
                            >
                                <div className={`w-8 h-5 rounded-full flex items-center px-0.5 transition-colors ${webSearch ? 'bg-purple-500' : 'bg-[var(--surface-active)]'}`}>
                                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${webSearch ? 'translate-x-3' : 'translate-x-0'}`} />
                                </div>
                                <span className="text-[var(--text-secondary)]">Web search</span>
                            </button>
                            <button 
                                ...
                                className="flex items-center gap-2 text-sm"
                            >
                                <div className={`w-8 h-5 rounded-full flex items-center px-0.5 transition-colors ${agentKnowledge ? 'bg-purple-500' : 'bg-[var(--surface-active)]'}`}>
                                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${agentKnowledge ? 'translate-x-3' : 'translate-x-0'}`} />
                                </div>
                                <span className="text-[var(--text-secondary)]">Agent knowledge</span>
                            </button>
