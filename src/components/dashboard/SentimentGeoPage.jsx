...
const getPromptPerformance = (user) => {
    ...
    return [
        { prompt: `Best ${industry.toLowerCase()} companies`, citations: 48, sentiment: 92, region: user?.location || 'US', platform: 'ChatGPT', trend: '+18%', positive: true },
        { prompt: `${industry} services review`, citations: 35, sentiment: 85, region: 'Global', platform: 'Perplexity', trend: '+12%', positive: true },
        { prompt: `Top ${industry.toLowerCase()} brands`, citations: 29, sentiment: 88, region: 'Europe', platform: 'Gemini', trend: '+8%', positive: true },
        { prompt: `${brandName} vs competitors`, citations: 22, sentiment: 76, region: 'Asia', platform: 'Claude', trend: '-2%', positive: false },
        { prompt: `${industry} market leaders`, citations: 18, sentiment: 81, region: user?.location || 'US', platform: 'ChatGPT', trend: '+25%', positive: true },
    ];
};
                        ...
                        <div key={i} className={`bg-gradient-to-br ${kpi.gradient} border ${kpi.border} rounded-2xl p-4`}>
                            ...
                            <div className="flex items-end justify-between">
                                ...
                                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium ${
                                   kpi.positive ? 'bg-[var(--surface-active)] text-[var(--text-primary)]' : 'bg-red-500/10 text-red-400'
                                }`}>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-[var(--border)]">
                                ...
                                <th className="text-center text-[10px] text-[var(--text-muted)] uppercase tracking-wider pb-3 font-semibold">Citations</th>
                                <th className="text-center text-[10px] text-[var(--text-muted)] uppercase tracking-wider pb-3 font-semibold">Sentiment</th>
                                <th className="text-center text-[10px] text-[var(--text-muted)] uppercase tracking-wider pb-3 font-semibold">Region</th>
                        ...
                        <tbody>
                            {getPromptPerformance(user).map((prompt, i) => (
                                <tr key={i} className="border-b border-[var(--border)] hover:bg-[var(--surface-hover)] transition-colors cursor-pointer">
                                    ...
                                    <td className="text-center text-[var(--text-primary)] text-sm">{prompt.citations}</td>
                                    <td className="text-center">
                                        <div className="inline-flex items-center gap-1.5">
                                            <div className="w-16 h-1.5 bg-[var(--surface-active)] rounded-full overflow-hidden">
                                                <div className="h-full rounded-full" style={{ 
                                                    width: `${prompt.sentiment}%`,
                                                    backgroundColor: prompt.sentiment > 80 ? '#ffffff' : prompt.sentiment > 60 ? '#737373' : '#ef4444'
                                                }} />
                                            </div>
                                            <span className="text-[var(--text-secondary)] text-xs">{prompt.sentiment}%</span>
                                        </div>
                                    </td>
                                    <td className="text-center">
                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[var(--surface-hover)] rounded-md text-[var(--text-secondary)] text-xs">
