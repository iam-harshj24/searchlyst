...
/**
 * Sends a 6-digit OTP to the user's email during the signup flow.
 */
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Verify Your Email – Searchlyst</title>
</head>
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;background-color:#0a0a0a;color:#ffffff;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#0a0a0a;">
    <tr>
          ...
          <tr>
            <td style="padding-top:40px;">
              ...
              <p style="margin:0 0 32px;font-size:16px;line-height:1.7;color:#a3a3a3;">
                Use the verification code below to complete your Searchlyst sign-up. This code expires in <strong style="color:#ffffff;">10 minutes</strong>.
              </p>
            </td>
          </tr>
          <!-- OTP Box -->
          <tr>
            <td style="padding-bottom:32px;">
              <div style="background-color:#171717;border:1px solid rgba(220,38,38,0.4);border-radius:12px;padding:32px;text-align:center;">
                <p style="margin:0 0 8px;font-size:13px;letter-spacing:0.08em;text-transform:uppercase;color:#737373;">Your verification code</p>
...
Your Searchlyst verification code is: ${otp}
...
www.searchlyst.com`;
  ...
  try {
    const info = await transporter.sendMail({
      ...
      to: email,
      replyTo: process.env.REPLY_TO_EMAIL || process.env.SMTP_USER,
      subject: `${otp} is your Searchlyst verification code`,
      text: textContent,
      html: htmlContent,
      headers: { 'X-Entity-Ref-ID': 'otp-signup' },
    });
